...
def compound_simulation(trades: list[dict], risk_per_trade_pct: float,
    ...
    # ★ 결함 #6 + #7 수정 (2026-05-17 서브에이전트 A 발견)
    # 이전: 모든 거래 즉시 순차 복리 (자본 회전 무한 + 동시 한도 무시)
    # 수정: 동시 max_concurrent (봇 본체 = 2) + active 큐 추적
    n_skipped = 0
    active = []  # [(end_ts, R)]
    max_concurrent = 2  # 봇 본체 동시 한도 (krtky_alert_bot.py)
    ...
    for t in trades:
        ...
        r = t["final_r"]
        # 자본 % 변화 = (risk / N_concurrent) × R 분할
        position_share = 1.0 / max_concurrent  # 동시 2 = 각 50%
        capital *= (1 + (risk_per_trade_pct / 100) * r * position_share)
        if r > 0: wins += 1
        elif r < 0: losses += 1
