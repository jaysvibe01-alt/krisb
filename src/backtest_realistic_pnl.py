...
def collect_entries(symbol: str) -> tuple[list[dict], int]:
    ...
    for ev in c.events:
        ...
        # ★ 결함 #5 최종 모델 (2026-05-17):
        # 매수존 [low ~ close] 안에 다음봉 wick 이 들어왔을 때만 fill.
        # 진입가 = 매수존 안 wick 최저점 (Long) / 최고점 (Short)
        # 라이브 limit 을 zone_low (Long) / zone_high (Short) 에 건 보수 시뮬.
        # 다음봉 wick 이 매수존 안 가면 fill 안 됨 = 좋은 자리 일부 놓침,
        # 단 fill 된 자리는 진짜 매수존 안에서 잡은 자리라 신뢰도 ↑.
        if direction == "long":
            if next_bar["low"] > zone_high:
                continue  # 갭 상승 — fill 안 됨
            # 다음봉 low 가 매수존 안에 들어왔으면 그 가격 또는 zone_low 중 더 좋은 가격
            entry_price = max(next_bar["low"], zone_low)
        else:
            if next_bar["high"] < zone_low:
                continue
            entry_price = min(next_bar["high"], zone_high)
